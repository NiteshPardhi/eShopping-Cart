...
export class CartService {
  ...
  productArray = [
    {
      id: 1, description: 'The best quality ever india', price: 20, name: 'Jins', img: 'assets/jins.jpeg'
    },
    {
      id: 2, description: 'Perfect to all size', price: 90, name: 'Sirt', img: 'assets/sirt.jpg'
    },
    {
      id: 3, description: 'The best stylish for men', price: 50, name: 'Watch', img: 'assets/watch.jpg'
    },
    {
  ...
  constructor() { 
    ...
    if(existProd){
      ...
      let totalProd = checkData.map((item: any) => {
  ...
  addToCart(product: any, qty: number) {
    let index = this.addeddProductList.findIndex((x: any) => x.name == product.name);
    if (index == -1) {
    ...
    localStorage.setItem('cartAddedItem', JSON.stringify(this.addeddProductList.slice()));
